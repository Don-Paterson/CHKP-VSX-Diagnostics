#!/bin/bash
# vsx_diagnostics.sh  v18
# VSX Gateway & Cluster Health Diagnostics with Topology Mapping
# Requires: Expert mode (root), VS0 context
# Usage: ./vsx_diagnostics.sh [-o logfile] [-f] [-q]
#   -f  Run 'vsx fetch' before diagnostics (required for topology map via vsx showncs)
#   -q  Quiet mode: log full output to file but print only executive summary to terminal
#
# Log file defaults to the same directory as the script (not /var/log).
...
DEFAULT_LOG="${SCRIPT_DIR}/vsx_diag_${TIMESTAMP}.log"

usage() {
    echo "Usage: $(basename "$0") [-o logfile] [-f] [-q]"
    echo "  -o logfile   Write output to logfile (default: <script dir>/vsx_diag_<timestamp>.log)"
    echo "  -f           Run 'vsx fetch' before diagnostics (required for topology map)"
    echo "  -q           Quiet: log everything to file, print only executive summary to terminal"
...
if [ "$QUIET" = "true" ]; then
    # Full output goes to log file only; terminal will receive summary via explicit echo later
    exec > "$LOGFILE" 2>&1
    # Print minimal progress to the original terminal via /dev/tty
    echo "VSX Diagnostics v${SCRIPT_VERSION} - logging to: $LOGFILE" > /dev/tty
    echo "Running in quiet mode - executive summary will be printed when complete." > /dev/tty
else
...
if [ -f "$VSALL_FILE" ]; then
    ...
    for member in $CLUSTER_MEMBERS; do
        ...
        MEMBER_SYNC_IP=$(grep "^\[${member}:\]interface set dev eth2 " "$VSALL_FILE" \
            | sed -n 's/.*address \([0-9][0-9.]*\).*/\1/p' | head -1)
        # Sanitize member name for use as shell variable (replace hyphens with underscores)
        member_var=$(echo "$member" | tr '-' '_')
        eval "MEMBER_${member_var}_MGMT=\"${MEMBER_MGMT_IP:-?}\""
        eval "MEMBER_${member_var}_SYNC=\"${MEMBER_SYNC_IP:-?}\""
        MARKER=""
...
# Probe showncs once to verify it is usable in this context.
# Use file redirection (not $() subshell) - vsx showncs suppresses stdout in subshells.
_showncs_probe=$(mktemp /tmp/vsx_showncs_probe.XXXXXX)
vsx showncs 1 > "$_showncs_probe" 2>/dev/null || true
if [ ! -s "$_showncs_probe" ]; then
    echo "WARNING: 'vsx showncs' returned no output (tested on VSID 1)."
    echo "         Topology map and WARP diagram sections will be skipped."
    echo "         To investigate manually: run 'vsx showncs 1' from VS0 expert mode."
    SHOWNCS_AVAILABLE=false
else
    SHOWNCS_AVAILABLE=true
fi
rm -f "$_showncs_probe"
unset _showncs_probe

for vs in $ALL_VSIDS; do
    [ "$vs" -eq 0 ] && continue
    if [ "$SHOWNCS_AVAILABLE" = "true" ]; then
...
if [ "$SHOWNCS_AVAILABLE" = "false" ]; then
    echo "  [skipped - vsx showncs not available in this context; see warning above]"
else
...
fi  # end SHOWNCS_AVAILABLE block
...
fw ctl affinity -l 2>&1 | sort -u || echo "  [unavailable]"
...
collect_vs_diag() {
            ...
            *)
                if [ "$rx_next" = "1" ]; then
                    ...
                    if [ "${rx_err:-0}" != "0" ] || [ "${rx_drop:-0}" != "0" ]; then
                        echo "IFACE_ERRORS|${vs}|${current_dev}|rx_err=${rx_err}|rx_drop=${rx_drop}|rx_pkts=${rx_pkts:-0}"
                    fi
                elif [ "$tx_next" = "1" ]; then
                    tx_next=0
                    tx_pkts=$(echo "$line" | awk '{print $2}')
                    tx_err=$(echo "$line" | awk '{print $3}')
                    tx_drop=$(echo "$line" | awk '{print $4}')
                    if [ "${tx_err:-0}" != "0" ] || [ "${tx_drop:-0}" != "0" ]; then
                        echo "IFACE_ERRORS|${vs}|${current_dev}|tx_err=${tx_err}|tx_drop=${tx_drop}|tx_pkts=${tx_pkts:-0}"
                    fi
                fi
                ;;
        esac
    ...
    if [ "$vtype" = "Virtual Switch" ]; then
        ...
        # Normalise verbose R82 context messages to short labels
        case "$blades" in
            "Virtual Switch context does not support software blades"*)
                blades="n/a (vsw-ctx)" ;;
            "Virtual Router context does not support software blades"*)
                blades="n/a (vr-ctx)" ;;
        esac
    fi
    ...
    echo "SUMMARY_DATA|${vs}|n/a|${mem_pct:-n/a}|${conn_now:-0}|${saccel:-n/a}|${blades:-n/a}|${mem_used_mb:-0}|${mem_total_mb:-0}|${swap_used:-0}|${idle_pct:-n/a}"
}
...
if cphaprob stat &>/dev/null 2>&1; then
    ...
    # Parse failover and state change events for attention
    FAILOVER_COUNT=$(echo "$CPHAPROB_OUTPUT" \
        | sed -n 's/.*Failover counter:[[:space:]]*\([0-9]*\).*/\1/p' | head -1)
    FAILOVER_TRANSITION=$(echo "$CPHAPROB_OUTPUT" \
        | sed -n 's/.*Transition to new ACTIVE:[[:space:]]*/  /p' | head -1)
    FAILOVER_TIME=$(echo "$CPHAPROB_OUTPUT" \
        | sed -n '/Last cluster failover event/,/^[[:space:]]*$/{
            s/.*Event time:[[:space:]]*//p
          }' | head -1)
    LAST_STATE_CHANGE=$(echo "$CPHAPROB_OUTPUT" \
        | sed -n 's/.*State change:[[:space:]]*//p' | head -1)
    LAST_STATE_CHANGE_TIME=$(echo "$CPHAPROB_OUTPUT" \
        | sed -n '/Last member state change/,/^[[:space:]]*$/{
            s/.*Event time:[[:space:]]*//p
          }' | head -1)
    ...
    echo "$CPHAPROB_OUTPUT" | grep -E '^\s*[0-9]' | while IFS= read -r line; do
        ...
        mstate=$(echo "$line" | awk '{for(i=1;i<=NF;i++) if($i ~ /ACTIVE|STANDBY|BACKUP|DOWN|READY/) print $i}')
    ...
    SYNC_LOST=$(echo "$SYNC_OUTPUT" | sed -n 's/.*Lost updates\.*[[:space:]]\([0-9]*\)/\1/p' | tr -d ' ')
    ...
    # Check for any non-OK PNOTEs (scope strictly to the Problem Notification table)
    PNOTE_ISSUES=$(echo "$CPSTAT_HA" | awk -F'|' '
        ...
        /\|/ && !/Name/ && !/---/ && !/^$/ {
            ...
            if (name != "" && status != "" && status != "OK" && status != "Status")
                print name ":" status
        }
    ')
else
...
printf "  %-4s %-16s %-5s %-10s %-6s %-12s %s\n" \
    "VS" "Name" "Type" "SecureXL" "Mem%" "Conns/Limit" "Blades"
printf "  %-4s %-16s %-5s %-10s %-6s %-12s %s\n" \
    "----" "----------------" "-----" "----------" "------" \
    "------------" "--------------------------------------"
...
# In quiet mode, redirect everything from here to end to both log (already going there)
# and terminal via /dev/tty
if [ "$QUIET" = "true" ]; then
...
for member in $CLUSTER_MEMBERS; do
    member_var=$(echo "$member" | tr '-' '_')
    eval "m_mgmt=\${MEMBER_${member_var}_MGMT:-?}"
...
for vs in $VS_GW_IDS; do
    ...
    wrp=""
    wrpj=""
    if [ "$SHOWNCS_AVAILABLE" = "true" ] && [ -s "$NCS_FILE" ]; then
        wrp=$(grep 'warp create' "$NCS_FILE" | sed -n 's/.*name_a \([^ ]*\).*/\1/p' | head -1)
        wrpj=$(grep 'warp create' "$NCS_FILE" | sed -n 's/.*name_b \([^ ]*\).*/\1/p' | head -1)
    fi
...
if [ -n "$FLOW_PARTS" ]; then
    ...
    if [ "$SHOWNCS_AVAILABLE" = "true" ]; then
        ...
        echo "  Both firewalls connect to ${swname:-VSW} (WARP interface names unavailable - vsx showncs not usable)."
    fi
fi
echo ""
...
if [ "${FAILOVER_COUNT:-0}" -gt 0 ] 2>/dev/null; then
    echo "  Cluster failover: ${FAILOVER_COUNT} since last reset"
    add_attention "Cluster failover occurred: ${FAILOVER_TRANSITION:-unknown transition} at ${FAILOVER_TIME:-unknown time} (failover count: ${FAILOVER_COUNT})"
else
    echo "  Cluster failover: none since last reset"
fi

# Degraded state (ACTIVE! or DOWN in "from" state of last state change)
if echo "${LAST_STATE_CHANGE:-}" | grep -q '(!)'; then
    add_attention "Cluster degraded state detected: '${LAST_STATE_CHANGE}' at ${LAST_STATE_CHANGE_TIME:-unknown time} (now resolved)"
elif echo "${LAST_STATE_CHANGE:-}" | grep -qE '^DOWN\b'; then
    add_attention "Cluster member was DOWN: '${LAST_STATE_CHANGE}' at ${LAST_STATE_CHANGE_TIME:-unknown time} (now resolved)"
fi
...
echo "  Memory: ${mem_pct:-?} used (${mem_used:-?}/${mem_total:-?} MB), swap: ${swap_used:-0} MB used${swap_used:+ (threshold: 100 MB)}"
if [ -n "$swap_used" ] && [ "${swap_used:-0}" -gt 100 ] 2>/dev/null; then
    add_attention "Swap in use: ${swap_used} MB (above 100 MB threshold)"
fi
...
# Interface errors
if [ -s "$TMPERRORS" ]; then
    # Build a set of monitored interfaces from cphaprob -a if output (cached in CPHAPROB_OUTPUT)
    MONITORED_IFACES=$(cphaprob -a if 2>/dev/null \
        | awk '/^[[:space:]]*[a-z]/ && !/Non-Monitored/ {gsub(/[[:space:]].*/, ""); print $1}')

    while IFS='|' read -r tag vs dev detail1 detail2 detail3; do
        eval "vname=\${VS_${vs}_NAME:-VS$vs}"
        err_val=$(echo "$detail1" | sed 's/.*=\([0-9]*\)/\1/')
        pkt_val=$(echo "$detail3" | sed 's/.*=\([0-9]*\)/\1/')
        # Check if interface is cluster-monitored
        mon_note=""
        if ! echo "$MONITORED_IFACES" | grep -qx "$dev" 2>/dev/null; then
            mon_note=" [not cluster-monitored - will not trigger failover]"
        fi
        if [ -n "$pkt_val" ] && [ "${pkt_val:-0}" -gt 0 ] 2>/dev/null && \
           [ -n "$err_val" ] && [ "${err_val:-0}" -gt 0 ] 2>/dev/null; then
            err_pct=$(awk "BEGIN {printf \"%.2f\", ($err_val/$pkt_val)*100}")
            add_attention "VSID $vs ($vname) $dev: $detail1 $detail2 / ${pkt_val} packets (${err_pct}% error rate)${mon_note}"
        else
            add_attention "VSID $vs ($vname) $dev: $detail1 $detail2${mon_note}"
        fi
    done < "$TMPERRORS"
fi
